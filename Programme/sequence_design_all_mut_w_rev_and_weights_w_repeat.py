...
class Sequence:
    ...
    def _process(self, nucleotid):
        ...
        x_seq = x_slide.reshape(x_slide.shape[0], x_slide.shape[1], self.WX, 4, 1)
        x_seq = x_seq.reshape(
            6 * self.length * self.length * self.repeat, self.WX, 4, 1
        )
        return x_seq
...
class Energy:
    def __init__(self, seq, model, y_target, length, repeat):
        ...
        (self.energy_profile, self.reg_energy_profile, self.rev_energy_profile) = (
    ...
    def _get_corr(self, y):
        ...
        return sigma_XY / (sigma_X * sigma_Y + (7.0 / 3 - 4.0 / 3 - 1))  # () is epsilon
    ...
    def _get_one_reverse_energy(self, y):
        return (
            5 * np.mean(np.abs(y - self.reverse_y_target))
            - self._get_reverse_corr(y)
            + 1
        )

    def _get_energy(self, y):
        return np.mean([self._get_one_energy(y[:, i]) for i in range(y.shape[1])])

    def _get_reverse_energy(self, y):
        return np.mean(
            [self._get_one_reverse_energy(y[:, i]) for i in range(y.shape[1])]
        )
    ...
    def _get_energy_profile(self, y):
        ...
        reg_energy = np.array([self._get_energy(y[i, :]) for i in range(mid_length)])
        rev_energy = np.array(
            [self._get_reverse_energy(y[i, :]) for i in range(mid_length, y.shape[0])]
        )
...
def _parse_arguments(args=None):
    ...
    parser.add_argument(
        "-l", "--length", default=167, help="""the length of the artificial sequence"""
    )
    parser.add_argument(
        "-r",
        "--repeat",
    ...
    parser.add_argument(
        "-s", "--steps", default=100, help="""Number of KMC iterations."""
    )
    parser.add_argument(
        "-m",
        "--model",
        default=f"{os.path.join('..', 'Results_nucleosome', 'weights_with_rev_compl_rep2.hdf5')}",
...
def main(command_line_arguments=None):
    ...
    y_target_ = shift + amp * np.exp(
        -((x_gauss - ((len601 - 1) / 2.0)) ** 2 / (2.0 * len601 * len601 / 16.0))
    )
    # linkers at last value
    y_target_ = np.append(
        y_target_, np.repeat(y_target_[-1], (int(args.length) - (len601 - 1)))
    )
    y_target = y_target_
    ...
    energy = Energy(
        sequence.seq_predictable, model, y_target, int(args.length), int(args.repeat)
    )
    ...
    with open(
        ...
        f_head.write(
            "length\trepeat\tstep\tmodel\ttemperature\tdirectory"
    ...
    for i in range(int(args.steps)):
        accepted = step(
            sequence, energy, float(args.temperature), count_mutation, args.weight
        )
        sequence.sequence = sequence.mutated_seq[accepted, :]
        mean_prediction = np.mean(energy.prediction_profile[accepted, :, :], axis=1)
        mean_com_prediction = np.mean(
            energy.prediction_profile[accepted + 3 * sequence.length, :, :], axis=1
        )
        mean_rev_prediction = np.flip(mean_com_prediction, 0)  # flipped
        ...
        if i % 1 == 0:  # plot every 1 step
            ...
            ax_pred.set_ylim(
                min(np.min(mean_prediction), np.min(y_target)) - 0.05,
                max(np.max(mean_prediction), np.max(y_target)) + 0.05,
            )
